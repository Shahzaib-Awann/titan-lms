...
const RightQuizSection = memo(function RightQuizSection({
  ...
  return (
    <aside className="flex h-full flex-col p-5">
      ...
      <div className="grid flex-1 min-h-0 auto-rows-max grid-cols-6 items-start justify-items-start gap-5 overflow-y-auto p-2">
        {questions.map((question, index) => (
          <QuestionNavigationButton
            ...
            isCurrentPage={index >= pageStart && index < pageEnd}
  ...
  return (
    <Button
      ...
      className={cn(
        ...
        isAnswered && "border-green-500 bg-green-500/10 text-green-500",
        !isAnswered &&
          "border-muted-foreground/30 bg-background text-muted-foreground",
        isCurrentPage && "ring-1 border-transparent ring-blurple ring-offset-1",
      )}
    >
      {index + 1}
    </Button>
